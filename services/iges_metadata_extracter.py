...
# --- Helper: count topological elements
...
# --- Helper: fuse list of shapes into one
...
def extract_iges_metadata(file_path):
    shapes = read_iges_file(file_path)
    if not shapes:
        raise ValueError("No shapes found in IGES file.")

    shape = fuse_shapes(shapes)
    if shape.IsNull():
        raise ValueError("Fused shape is null.")
...
# --- Runner
def main():
    input_dir = "iges_files"  # keep using same folder
    output_dir = "output_metadata"
    os.makedirs(output_dir, exist_ok=True)

    for fname in os.listdir(input_dir):
        if fname.lower().endswith((".igs", ".iges")):
            path = os.path.join(input_dir, fname)
            try:
                print(f"Processing {fname}")
                meta = extract_iges_metadata(path)
